...
These `.md` files are consumed by the `workflow.ts` / `workflow-team.ts`
...
pi
/workflow                           # pick a team (Select Team dialog), then type the request
```
...
- `/workflow [request]` — single-model lifecycle (every agent uses the session model)
- `/workflow-team [request]` — per-agent models (env / `models.yaml`); see `../extensions/README.md`
