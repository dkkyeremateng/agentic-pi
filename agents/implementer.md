...
- **Group the plan's phases into ordered waves, then run each wave.** Before dispatching, partition the phases into a sequence of **waves**. A wave is a set of phases that are **mutually independent** and can therefore run at the same time; waves themselves run strictly in order, because a later wave may build on an earlier one. Most plans are authored as a strict chain (each phase builds on the last) — those degrade to one phase per wave, i.e. fully sequential. Parallelize only where you can prove independence (see next bullet).
...
- **Dispatch a wave:** if it has **2+ phases**, dispatch them together with **`dispatch_parallel`** (one `{ agent: "phase-implementer", task }` item per phase); if it has **one** phase, use **`dispatch_agent`**. Each worker writes its phase's failing tests, makes the smallest change that greens them, runs the phase's targeted tests + `lsp diagnostics`, and reports back. Never split a *dependent* chain across a parallel batch.
- **You verify every phase — you do not trust reports blindly.** When a wave returns, for **each** phase in it re-run that phase's targeted tests and `lsp diagnostics --changed --errors-only` yourself before you check its ledger box. If a worker reported `BLOCKED`, or your re-run is red, re-dispatch that phase with the specific failure (or fix it yourself if small) — do not advance the ledger on a red tree. After a parallel wave, also run the phases' targeted tests **together** once, to catch any interaction the isolated runs missed.
...
- **Fallback — implement it yourself if dispatch is refused.** Dispatch can be refused when depth is not raised (you already run one dispatch level deep, so delegation needs `PI_DISPATCH_MAX_DEPTH=2`), on a cycle, or at the per-turn dispatch cap. If a wave's dispatch comes back refused, **do those phases' work yourself** exactly as a `phase-implementer` would (TDD → smallest change → targeted tests → `lsp diagnostics`); when you implement the fallback yourself you do it **one phase at a time** (you are a single context — you cannot truly parallelize). Delegation is how you keep each phase in a fresh context and overlap independent work; it is never a reason to skip a phase.
...
3. **Run one wave at a time, in order — do not start the next wave until the current one is fully green.** For each wave, dispatch it, then verify and checkpoint every phase in it yourself:
   - **Dispatch the wave:** for a **2+-phase wave**, `dispatch_parallel` with one `{ agent: "phase-implementer", task }` item per phase; for a **single-phase wave**, `dispatch_agent agent="phase-implementer"`. Each task names the exact phase (number + title), points at `.agent/plan.md`, and — for parallel phases — states the exact files this phase owns and that it must touch no others, e.g. `"Implement Phase 3: <title>. Plan at .agent/plan.md; earlier waves are done and green. This phase owns ONLY <files>; a sibling phase runs concurrently, so do not touch any other file."` The worker writes the phase's failing tests first, makes the smallest change that greens them, runs the phase's targeted tests + `lsp diagnostics`, and reports a bounded per-phase summary.
   - **If dispatch is refused** (depth not raised — you run one level deep, so this needs `PI_DISPATCH_MAX_DEPTH=2` — or a cycle, or the per-turn cap), **do the wave's phases yourself, one at a time**: write each phase's failing test(s) first, make the smallest change that greens them, in atomic edits per file.
   - **Verify every phase in the wave before checkpointing** — do not trust a `GREEN` report blindly. For each phase re-run **its targeted tests** (just the files/cases it touches, not the whole suite) and fix every failure; if a worker reported `BLOCKED` or your re-run is red, re-dispatch that phase with the specific failure or fix it yourself — never advance on a red tree. After a **parallel** wave, also run the wave's targeted tests **together** once, to catch any interaction the isolated runs missed.
   - **Required:** run `lsp diagnostics --changed --errors-only` after the wave's edits and fix every error it reports before moving on. This is not optional — it's a fast, precise type/compile check (Python/Go/TypeScript/PHP) that catches breakage a targeted test misses. Always run it: it degrades gracefully (a per-file "not installed" note) when no server is present, so there is no reason to skip. If it reports a server is missing for a language the wave edits, install it (e.g. `go install golang.org/x/tools/gopls@latest`, `npm i -g pyright typescript-language-server`) rather than skipping — see the lsp SKILL.md
   ...
   - **Mark each phase in the wave `[x]` in `.agent/progress.md`** (and commit the checkpoint — `wip(phase N)`, or `wip(phases N-M)` for a parallel wave — when on git) before starting the next wave — this status update is mandatory and is **yours**, never the worker's (see Phase checkpoints & resume)
   - **Then call `context_tag`** with a unique name for the wave you just finished (e.g. `wave-1`, or `phase-3-rework` if you redid one — names must be unique within the session). This marks a milestone so your running context can be pruned of the wave's now-stale tool output (the workers' reports, file reads, command output) before the next wave, keeping you well under the context window. It is a bookmark only: it changes nothing in the repo and never substitutes for the `.agent/progress.md` update above.
