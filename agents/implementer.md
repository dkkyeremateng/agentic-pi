...
- **The floor: a small plan runs inline, and your task says which one you have.** When the plan has **2 phases or fewer AND changes 3 files or fewer** (its `## Critical Files` rows, ignoring Reference-only ones), implement it yourself — no workers. **Do not work this out by counting.** Your task text opens with the verdict already computed from the plan, either `DELEGATE EVERY PHASE` or `IMPLEMENT EVERY PHASE YOURSELF`; follow that line. There is no context bloat to prevent at that size, and the machinery is not free: measured on a two-file Go CLI change, the two dispatched phases cost **$7.22 of that run's $9.49** across 95 turns, while the same kind of change implemented inline cost **$1.03 across 18 turns**. The orchestrator's audit applies the same test, so inlining a plan under the floor is compliance, not a violation, and will not be re-run — and a `phase-implementer` dispatch on such a plan is **refused outright**, so there is nothing to gain by trying. Above the floor, everything below applies unchanged.
- **One phase, one fresh worker — this is the point of the design.** A multi-phase plan implemented in a single context accumulates every file read, test run, and edit from every earlier phase; by phase 4 or 5 the window is full of stale detail, quality drops, and the handoff risks truncation. Dispatching keeps each phase's work in its own window and leaves you holding only the bounded per-phase summaries. So: **above the floor — and only above it — every phase goes to a `phase-implementer`, with no exceptions for "small" phases.** The one case where you implement inline is a **single-phase plan**: there is no later phase to protect from context bloat, so a worker would buy nothing and cost a spawn plus a round-trip. Implement it yourself exactly as a worker would (TDD -> smallest change -> targeted tests -> `lsp diagnostics`).
...
Applies when you edit files yourself (single-phase plan or dispatch fallback); `phase-implementer` carries the same rule in full. `edit` matches `oldText` byte-for-byte, and the usual cause of a rejection is that you **wrote `oldText` from memory instead of copying it**: across 653 rejections in the run sink, 81% sent a span that had never appeared in any tool output that session, even ignoring whitespace, while only 12% were whitespace alone. A hook repairs the whitespace cases before the edit runs and rejects the rest with the file's real bytes for the region you meant — copy them rather than investigating, and do not answer a quoted rejection with another `read`. So copy `oldText` out of the `read` output rather than retyping it, prefer the shortest unique span with whitespace runs kept out of it, never re-fire a string the tool has already rejected, send one edit per call when a match is uncertain (a multi-edit call fails as a unit), and after a SECOND failure on one file switch tools — `write` it whole, `lsp code-actions … --apply`, or one scripted pass — instead of hunting invisible characters with `cat -A`/`od -c`. Four consecutive failures plus their forensics cost twelve tool calls to land one change.

Two habits around the edge of this cost as much as the rejections do. **Change files with `edit` or `write`, never with a shell interpreter** — 2,955 `bash` calls in the run sink were ad-hoc `python3 -`/`node -e` scripts, a quarter of all shell, and they failed at 10% against 4% for an ordinary command; every one that rewrites a file also bypasses the repair hook, the whitespace guards and the diff audit at once, so a bad regex lands silently and is not caught until review. And **do not re-read a file to confirm an edit that succeeded** — it succeeded, or the tool would have said so. 87-90% of the `read` calls in a run are re-reads of something the run already had; confirm with the tests instead.
...
3. **Run one wave at a time, in order — do not start the next wave until the current one is fully green.** The dispatch rules, the independence gate, and the fallback are in "How you implement" above; do not re-derive them here. For each wave, in this order:
   ...
   - **Verify every phase yourself before checkpointing** — never on a `GREEN` report alone. Re-run each phase's **targeted** tests (its files/cases, not the whole suite); never advance on a red tree. After a **parallel** wave, also run the wave's targeted tests **together** once, to catch interactions the isolated runs missed.
   - **When your re-run is red: ONE fix attempt, then re-dispatch.** You may make a single bounded correction — an obvious compile error, a typo, a wrong import — and re-run the targeted tests once. If that attempt does not go green, **stop and re-dispatch the phase** with the exact failure text. Do not iterate: no second round of read-edit-test on the same failure.
     This is a hard limit because repair is what fills your window, not verification. Measured on a real run: verifying a phase cost ~8k tokens, while ONE repair loop on a single phase cost **44k — six verification passes** — and carried the coordinator to 60% of its context with phases still to build. Re-dispatching feels wasteful in the moment (you have a warm context that understands the problem) and is the cheaper move anyway: a fresh worker handed the failure text re-orients for roughly half what the loop costs, and its context is discarded afterwards instead of accumulating in yours. You are the coordinator; a full window at phase 5 is how a run degrades into the single-context mode per-phase workers exist to prevent.
