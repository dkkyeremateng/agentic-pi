...
**The usual cause is that you wrote `oldText` from memory rather than copying it.** Classifying 653 rejections across the run sink against every byte of tool output their session had ever received: 81% sent a span that had never appeared in ANY of it, even with all whitespace ignored. Only 12% were whitespace alone. On a Go repo it is starkest — of 117 rejected `oldText` values, **three contained a tab**, in a tree `gofmt` indents with tabs; multi-line Go edits carrying a tab fail 28% of the time, ones indented with spaces fail 61%. Whitespace is now repaired for you before the edit runs, and when the mismatch is not whitespace the rejection quotes the file's real bytes for the region you meant — so the rules below are about reproduction, which is the actual problem.
...
- **When a rejection quotes the file's actual text, that IS the answer — copy it.** A hook reads the file before your edit runs and rejects a doomed edit with the real bytes shown: the exact span when the difference is whitespace, otherwise the region you were aiming at, located by whichever of your lines does exist. Either way it is the file, so do not answer it with a `read`, and do NOT open a forensics loop (`cat -A`, `od -c`, repeated `python3` heredocs): in the measured run five such calls found nothing, because there was nothing invisible to find.
...
- **Change files with `edit` or `write`, not with a shell interpreter.** Across the run sink, 2,955 `bash` calls were ad-hoc `python3 -`/`node -e` scripts — a quarter of all shell — and they failed at **10% against 4% for an ordinary command**. Worse, every one that rewrites a file bypasses the repair hook, the whitespace guards and the diff audit together, so damage from a bad regex lands silently and is not found until review. Keep interpreters for computation that has no tool behind it.
- **Do not re-read a file to confirm an edit that succeeded.** It succeeded; the tool would have said otherwise. 87-90% of all `read` calls in a run are re-reads of a file the run already had, and a turn costs the same whatever is in it. Confirm with the tests.
