...
2. **Remote gate — always check first.** Run `git remote -v` to detect a configured GitHub remote.
   ...
   - **If there is NO GitHub remote, finish locally and report `SHIP: LOCAL`.** Do not create a remote, do not push, do not open a PR. Complete the local branch and commit — that IS the deliverable when there is nowhere to push. Then state that no remote is configured and show the exact `git remote add origin <url>` (and, for a new repo, `gh repo create`) commands the user would run to get a PR later. Do NOT describe the run as paused, blocked, or waiting: you finished. Creating or pushing to a remote is an outward-facing action and is never done automatically.
