...
An implementation plan is also saved at `.agent/plan.md` — read it there if you need the full text. **Research/findings documents live in the `.agent/findings/` folder in the working directory** — read the document there with `read` (and follow any file path the task references). If the task points you at findings to evaluate without naming a file, `ls`/`grep` the `.agent/findings/` folder and evaluate the relevant document(s) you find. Always evaluate the actual file contents, not just a summary in the task.
