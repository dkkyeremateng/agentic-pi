...
| Path | What it is |
|------|-----------|
| `extensions/` | pi extensions. `agent-workflow.ts` runs a self-healing **plan → refine → implement → review → validate → ship** workflow with per-agent models; `dispatch.ts` is a standalone extension owning the `dispatch_agent` / `dispatch_parallel` / `select_agents` tools (used by any agent, in any session). See [`extensions/README.md`](extensions/README.md). |
| `agents/` | Agent definitions (`.md` with frontmatter): `scout`, `planner`, `implementer`, `reviewer`, `validator`, `shipper`, plus `seeker` (browser/web), `linear` (issue tracking), and `atlassian` (Jira tickets). |
